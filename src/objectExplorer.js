...
// ── Assembly Diagnostic Summary ──
// Logs a compact report of assembly property detection results
function logAssemblyDiagnostic() {
  const total = allObjects.length;
  let withPos = 0, withName = 0, withCode = 0, withAny = 0;
  const sourceApps = {};
  ...
  for (const obj of allObjects) {
    ...
    // Count source apps
    const app = obj.sourceApp || "Unknown";
    sourceApps[app] = (sourceApps[app] || 0) + 1;
  }

  console.log(
    `[Assembly] ${total} objects: POS=${withPos} NAME=${withName} CODE=${withCode} ANY=${withAny} NONE=${total - withAny}`
  );

  // Log source application distribution
  const appList = Object.entries(sourceApps)
    .sort((a, b) => b[1] - a[1])
    .map(([app, count]) => `${app}=${count}`)
    .join(" ");
  console.log(`[SourceApp] ${appList}`);
}
